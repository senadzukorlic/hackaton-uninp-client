...
import { Check, Brain, Cpu, Cloud, Shield, Star, ChevronRight } from 'lucide-react';
  ...
  const pricingTiers: PricingTier[] = [
    {
      ...
      price: "Besplatno",
      billingPeriod: isAnnual ? '/mesec, naplaćuje se godišnje' : '/mesec, naplaćuje se mesečno',
    ...
    <section className="py-24 bg-gradient-to-b from-gray-50 to-white dark:from-gray-900 dark:to-gray-800 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        >
          ...
          >
            Skalirajte svoje AI mogućnosti sa našim fleksibilnim cenovnim opcijama
          </motion.p>
              ...
              <div className="p-8">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className={`mt-2 ${tier.highlight ? 'text-blue-100' : 'text-gray-500 dark:text-gray-400'}`}>
                      {tier.description}
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-8">
                  <div className="flex items-baseline">
                    ...
                    <span className={`ml-2 ${tier.highlight ? 'text-blue-100' : 'text-gray-500 dark:text-gray-400'}`}>
                      {tier.billingPeriod}
                    </span>
                  </div>
                    ...
                    {tier.features.map((feature) => (
                      <li key={feature.name} className="flex items-start">
                        <div className="flex-shrink-0">
                          {feature.included ? (
                            <Check className={`w-5 h-5 ${tier.highlight ? 'text-blue-200' : 'text-green-500 dark:text-green-400'}`} />
                          ) : (
                            <span className="block w-5 h-5 border-2 border-gray-300 dark:border-gray-600 rounded-full" />
                          )}
                        </div>
                        <span className={`ml-3 ${feature.included ? '' : (tier.highlight ? 'text-blue-100' : 'text-gray-500 dark:text-gray-400')}`}>
                          {feature.name}
                        </span>
                      </li>
                    ))}
            ...
            <div className="flex items-center justify-between flex-col md:flex-row gap-6">
              <div>
                ...
                <p className="mt-2 text-gray-600 dark:text-gray-300 max-w-2xl">
                  Potrebne su vam prilagođene bezbednosne funkcije ili zahtevi za usklađenost? Naš preduzetnički plan uključuje napredne bezbednosne mere, posvećenu podršku i prilagođena rešenja.
                </p>
              </div>
