...
const ChildInfoCard: React.FC<ChildInfoCardProps> = ({ name, iconColorClass, status, location }) => {
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 mb-4">
      ...
      <p className="text-sm text-gray-600 dark:text-gray-400">
        <span className="font-medium">Lokacija:</span> {location.lat.toFixed(4)}, {location.lng.toFixed(4)}
      </p>
    </div>
  );
};
...
const ParentControl: React.FC = () => {
  ...
  const [sonLocation, setSonLocation] = useState({ lat: 44.7866, lng: 20.4489 }); // Starting point (e.g., Belgrade)
  const [daughterLocation, setDaughterLocation] = useState({ lat: 44.7870, lng: 20.4495 }); // Slightly different start
  ...
  const internetClub2Location = { lat: 44.7700, lng: 20.4350 }; // Internet Klub 2 coordinates

  // Simulate children's movement
  useEffect(() => {
    // Son's movement towards Internet Klub 2
    const sonDestinations = [
      { lat: 44.7840, lng: 20.4460 }, // Step 1
      { lat: 44.7780, lng: 20.4400 }, // Step 2
      { lat: 44.7720, lng: 20.4360 }, // Step 3 (closer to Internet Klub 2)
      internetClub2Location, // Final destination
    ];

    // Daughter's movement towards School
    const daughterDestinations = [
        { lat: 44.7900, lng: 20.4520 }, // Step 1
        { lat: 44.7930, lng: 20.4550 }, // Step 2
        { lat: 44.7960, lng: 20.4580 }, // Step 3 (closer to School)
    ...
    const interval = setInterval(() => {
      ...
      if (step >= sonDestinations.length && step >= daughterDestinations.length) {
  ...
  useEffect(() => {
    const distanceToSchool = calculateDistance(
      sonLocation.lat, sonLocation.lng, schoolLocation.lat, schoolLocation.lng
    );
    const distanceToInternetClub = calculateDistance(
      sonLocation.lat, sonLocation.lng, internetClubLocation.lat, internetClubLocation.lng
    );
    const distanceToInternetClub2 = calculateDistance(
      sonLocation.lat, sonLocation.lng, internetClub2Location.lat, internetClub2Location.lng
    );
    ...
    if (distanceToInternetClub2 < alertThreshold) {
      setSonStatus(`Blizu Internet Kluba 2 (${distanceToInternetClub2.toFixed(0)}m)`);
      if (distanceToInternetClub2 < distanceToSchool) {
       ...
       setSonStatus(`Blizu Internet Kluba (${distanceToInternetClub.toFixed(0)}m)`);
    ...
    if(alert) {
        axios.post("http://localhost:8080/api/chat/warn", {
            "warning": alert
         }, {
            headers: {
  ...
  }, [sonLocation, schoolLocation, internetClubLocation, internetClub2Location]); // Recalculate when son moves or locations change

   // Update daughter's status based on location
  useEffect(() => {
    const distanceToSchool = calculateDistance(
      daughterLocation.lat, daughterLocation.lng, schoolLocation.lat, schoolLocation.lng
    );
     const distanceToInternetClub = calculateDistance(
      daughterLocation.lat, daughterLocation.lng, internetClubLocation.lat, internetClubLocation.lng
    );
     const distanceToInternetClub2 = calculateDistance(
      daughterLocation.lat, daughterLocation.lng, internetClub2Location.lat, internetClub2Location.lng
    );
    ...
    if (distanceToSchool < statusThreshold) {
      setDaughterStatus(`Blizu Škole (${distanceToSchool.toFixed(0)}m)`);
       ...
       setDaughterStatus(`Blizu Internet Kluba (${distanceToInternetClub.toFixed(0)}m)`);
    } else if (distanceToInternetClub2 < statusThreshold) {
        setDaughterStatus(`Blizu Internet Kluba 2 (${distanceToInternetClub2.toFixed(0)}m)`);
    }
     else {
      setDaughterStatus("Kretanje...");
    }
  }, [daughterLocation, schoolLocation, internetClubLocation, internetClub2Location]); // Recalculate when daughter moves
      ...
      <div className="w-full md:w-1/2 h-[50vh] md:h-screen p-4">
        ...
        <div className="h-[calc(100%-4rem)] w-full rounded-lg overflow-hidden shadow-lg border border-gray-200 dark:border-gray-700">
          ...
          >
            ...
            {/* Son's Marker */}
            <Marker position={[sonLocation.lat, sonLocation.lng]} icon={sonIcon}>
              <Popup>Moj Sin</Popup>
            </Marker>

             {/* Daughter's Marker */}
            <Marker position={[daughterLocation.lat, daughterLocation.lng]} icon={daughterIcon}>
            ...
            <Marker position={[internetClubLocation.lat, internetClubLocation.lng]} icon={internetClubIcon}>
              <Popup>Internet Klub</Popup>
            </Marker>

            {/* Internet Klub 2 Marker */}
            <Marker position={[internetClub2Location.lat, internetClub2Location.lng]} icon={internetClub2Icon}>
      ...
      <div className="w-full md:w-1/2 p-6 md:pt-10 overflow-y-auto">
        ...
        <div className="space-y-4">
         ...
         <div className="mt-8 pt-4 border-t border-gray-300 dark:border-gray-700">
             <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-3">Legenda Mape</h3>
             <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm text-gray-700 dark:text-gray-300">
                 <div className="flex items-center"><span className="w-3 h-3 rounded-full mr-2 bg-blue-500"></span> Sin</div>
                 <div className="flex items-center"><span className="w-3 h-3 rounded-full mr-2 bg-purple-500"></span> Ćerka</div>
                 <div className="flex items-center"><span className="w-3 h-3 rounded-full mr-2 bg-green-500"></span> Škola</div>
                 <div className="flex items-center"><span className="w-3 h-3 rounded-full mr-2 bg-red-500"></span> Internet Klub 1</div>
                 <div className="flex items-center"><span className="w-3 h-3 rounded-full mr-2 bg-orange-500"></span> Internet Klub 2</div>
             </div>
         </div>
